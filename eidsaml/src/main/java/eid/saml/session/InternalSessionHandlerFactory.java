...
import eid.saml.session.inmemory.InMemorySessionHandlerFactory;
...
/**
 * Internal factory for creating session handlers based on the configuration defined in eidsaml.properties
 */
public class InternalSessionHandlerFactory implements SessionHandlerFactory {
     ...
     *
     * @param config EIDSAML configuration
     */
    @Override
    public synchronized void configure(Configuration config) throws InitializationException {
        ...
        try {
            ...
            throw new InitializationException(String.format("Cannot create SessionHandlerFactory, '%s' must have default constructor and implement 'eid.saml.audit.AuditLogger'", clazz.getName()), e);
        }
    }
}
