...
public class DatabaseSessionHandler implements SessionHandler {
                ...
                try(ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        authnRequestWrapper = new AuthnRequestWrapper(
                                (AuthnRequest) StringUtil.base64ToXMLObject(rs.getString(1)), rs.getString(3));
                    }
                }
            }
    ...
    /**
     * Get EIDSAML session ID for session with session index
     *
     * @param sessionIndex Session index to lookup session ID for
     * @return EIDSAML session ID (for audit logging)
     */
    @Override
    ...
    public void cleanup(final long maxInactiveIntervalSeconds) {
        try (Connection connection=ds.getConnection()){
            ...
            try(PreparedStatement ps = connection.prepareStatement("SELECT session_id, assertion_id, subject_name_id FROM assertions_tbl WHERE access_time < ?")) {
                ...
                try(ResultSet rs = ps.executeQuery()) {
                    while(rs.next()) {
                        EIDSAMLService.getAuditService().auditLog(new AuditService
                                .Builder()
    ...
    private void logout(String sessionId) {
        log.debug("Invalidate EIDSAML session '{}'", sessionId);
        try (Connection connection=ds.getConnection()) {
            connection.setAutoCommit(true);
            ...
            log.warn("Unable to remove EIDSAML session '{}'", sessionId, e);
        }
    }
