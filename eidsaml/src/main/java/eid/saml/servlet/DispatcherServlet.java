...
import eid.saml.servlet.ErrorHandler.ERROR_TYPE;

public class DispatcherServlet extends HttpServlet {
    private static final long serialVersionUID = 7955038593749446991L;
    ...
    private void handleOptionalValues(Map<String, String> config, Configuration configuration) {
        String value = config.get(Constants.EIDSAML_VALIDATION_ENABLED);
        if (StringUtil.isNotEmpty(value)) {
            configuration.setValidationEnabled("true".equals(value));
        }

        value = config.get(Constants.EIDSAML_ASSURANCE_LEVEL_SKIP);
        if(StringUtil.isNotEmpty(value)) {
            configuration.setAssuranceLevelSkipped("true".equals(value));
        }

        value = config.get(Constants.EIDSAML_ASSURANCE_LEVEL_MINIMUM);
        if (StringUtil.isNotEmpty(value)) {
            try {                
                configuration.setMinimumAssuranceLevel(value);
            }
            catch (Exception ex) {
                log.warn("Invalid value {} = {}", Constants.EIDSAML_ASSURANCE_LEVEL_MINIMUM, value, ex);
            }
        }
        ...
        value = config.get(Constants.METADATA_CONTACT_COMPANY);
        if (StringUtil.isNotEmpty(value)) {
            configuration.setContactCompany(value);
        }

        value = config.get(Constants.METADATA_CONTACT_GIVEN_NAME);
        if (StringUtil.isNotEmpty(value)) {
            configuration.setContactGivenName(value);
        }

        value = config.get(Constants.METADATA_CONTACT_SURNAME);
        if (StringUtil.isNotEmpty(value)) {
            configuration.setContactSurName(value);
        }

        value = config.get(Constants.METADATA_CONTACT_TELEPHONE_NUMBER);
        if (StringUtil.isNotEmpty(value)) {
            configuration.setContactTelephoneNumber(value);
        }
